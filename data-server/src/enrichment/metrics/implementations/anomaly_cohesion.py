"""Cohesion anomaly metric — v2 port (Chunk 15a; activity traits deferred to 15b).

Port of legacy ``src/enrichment/tagger/anomaly_cohesion.py``. The legacy
emitted nine :attr:`TraitFamily.COHESION` traits across three
sub-families. Per Chunk-15 brief "split if >300 LOC", this file
(15a) ships **six** of the nine — the ``coordination`` and ``size``
sub-families. The ``activity`` sub-family (Hibernator / Awakening /
Erosion) is deferred to **Chunk 15b** because it carries the only
time-bucket dependency on ``file_trait_utils.time_bucketed_commits``
+ ``linear_slope`` and would push this file well past the LOC budget.

Sub-families shipped here:

* **coordination** — Bazaar, Cathedral, Pulsar, Flicker.
...
Consumes ``file_trait_utils.change_net_churn`` from Chunk 11.
``frequent_changer_recent_*`` evaluates against the recent cutoff
which mirrors the legacy semantics.
"""
...
from src.enrichment.utils.file_trait_utils import change_net_churn
...
class _CohesionConfig:
    """Cached snapshot of the 11 config knobs this metric reads."""
