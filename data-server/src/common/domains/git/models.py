...
class LineChange(BaseModel):
    ...
    Generated by ``@with_ref_resolvers`` (kernel/entity.py):
        .commit(graph) -> Optional[Commit]
    """
...
class Commit(Entity):
    ...
    * ``issues`` / ``pull_requests`` — DROPPED (cross-source links). Move
    ...
    Resolver methods (auto-generated, see ``kernel/entity.py``):
        ``.project(graph)``   -> ``GitProject | None``
        ``.author(graph)``    -> ``GitAccount | None``
        ``.committer(graph)`` -> ``GitAccount | None``
        ``.parents(graph)``   -> ``list[Commit]`` (unresolved entries dropped)
    """
...
class Change(Entity):
    ...
    * ``annotated_lines`` /
    ...
    Resolver methods (auto-generated, see ``kernel/entity.py``):
        ``.commit(graph)``        -> ``Commit | None``
        ``.file(graph)``          -> ``File | None``
        ``.parent_commit(graph)`` -> ``Commit | None``
        ``.parent_change(graph)`` -> ``Change | None``
        ``.hunks(graph)``         -> ``list[Hunk]``
    """
