...
class Comment(BaseModel):
    ...
    Generated by ``@with_ref_resolvers`` (kernel/entity.py):
        .author(graph)     -> Optional[JiraUser]
        .updated_by(graph) -> Optional[JiraUser]
    """
...
class JiraUser(Account):
    ...
    * ``issues_as_*``     — DROPPED. Reverse lookup via
    ...
    Resolver methods (auto-generated, see ``kernel/entity.py``):
        ``.project(graph)`` -> ``JiraProject | None``
    """
...
class Issue(Entity):
    ...
    * ``priority`` / ``resolution`` /
    ...
    Resolver methods (auto-generated, see ``kernel/entity.py``):
        ``.project(graph)``    -> ``JiraProject | None``
        ``.status(graph)``     -> ``IssueStatus | None``
        ``.type(graph)``       -> ``IssueType | None``
        ``.creator(graph)``    -> ``JiraUser | None``
        ``.reporter(graph)``   -> ``JiraUser | None``
        ``.assignees(graph)``  -> ``list[JiraUser]``
        ``.parent(graph)``     -> ``Issue | None``
    """
