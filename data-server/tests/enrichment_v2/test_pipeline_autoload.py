...
Verifies that importing :mod:`src.enrichment.v2_pipeline` ALONE is
enough to populate :data:`BUILDERS` and :data:`METRICS` — i.e. a
Chunk-8 caller that does ``from src.enrichment.v2_pipeline import
...
def test_importing_v2_pipeline_alone_populates_catalogs() -> None:
    """Cold-import ``v2_pipeline`` and assert both catalogs are non-empty.
    ...
    script = textwrap.dedent(
        ...
        sys.path.insert(0, ".")
        from src.enrichment.v2_pipeline import run_pipeline  # noqa: F401
        from src.enrichment.relations_v2 import BUILDERS
...
def test_run_pipeline_against_default_catalogs_emits_via_autoload() -> None:
    ...
    script = textwrap.dedent(
        ...
        from src.enrichment.v2_pipeline import run_pipeline
